...
const isUserRecruiter = (session) =>
  session.user && session.user.role === "recruiter";
...
export const createJob = async (req, res) => {
  try {
    ...
    let secureCompany = xss(company);
    let secureJobDescription = xss(job_description);
    let secureCompensation = xss(compensation);
    let secureMode = xss(mode);
    let secureCategory = xss(category);

    let job = {
      company: secureCompany,
      job_description: secureJobDescription,
      compensation: secureCompensation,
      mode: secureMode,
    ...
    if (!exists) {
      ...
      res.render("error", {
        message: "Failed to create job, duplicate entry",
        status: 409,
      });
    }
...
export const getJobs = async (req, res) => {
  try {
    ...
    jobs.forEach((job) => {
      job.userHasApplied = job.applicants.some(
        (applicant) => applicant.user_id.toString() === userId.toString()
      );
    });
    // jobs.forEach(job => {
...
export const likeJob = async (req, res) => {
  try {
    ...
    jobs.forEach((job) => {
      job.userHasLiked = job.likedBy.some(
    ...
    jobs.forEach((job) => {
      job.userHasDisliked = job.dislikedBy.some(
        (dislike) => dislike.user_id.toString() === userId.toString()
      );
